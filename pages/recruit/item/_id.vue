...
export default {
  aysncData ( {param} ) {
    return recruitApi.getCityDetail( param.id )
      .then( recruitRes=> {
        return enterpriseApi.getEnterpriseDeatil (recruitRes.data.data.id)
          .then( enterpriseRes=> {
            return {
              cityItem: recruitRes.data.data,
              enterpriseItem: enterpriseRes.data.data
            }
          })
